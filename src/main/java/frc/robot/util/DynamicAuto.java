...
public class DynamicAuto 
{
  ...
  /**
   * Splits a string of auto command phrases and gets the path command and robot
   * command associated with each command phrase
   * 
   * @param commandInput
   *                       The string of commands to split, seperated by commas
   *                       with no spaces (e.g. "a1,rA1,p,cR3")
   * @return An array of commands, from the input command phrase string, in the
   *         same order
   */
  public static ArrayList<Command> getCommandList(String commandInput) 
  {
    ...
    for (int i = 0; i < splitCommands.length; i++) 
    {
      ...
      } else if (splitCommands[i].charAt(0) == 'r') 
      {
        ...
        commandList.add(AutoBuilder.pathfindThenFollowPath(nextPath, DynamicAuto.constraints));
